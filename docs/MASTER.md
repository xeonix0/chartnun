...
| 시각화 색상 | 상승/롱=초록, 하락/숏=빨강, 이평선=파랑, 추세선=주황, 지지/저항=자주색, 박스권=보라 (스펙 §10-1-1 기준으로 확정, CLAUDE.md 표 수정 완료). 2026-07-08: 코단 대시보드 실사용 중 "선이 잘 안 보인다" 피드백으로 트레이딩뷰 다크 모드를 벤치마킹해 정확한 hex를 전면 교체(색상 계열은 무변경) — `#131722`(배경)/`#2A2E39`(그리드)/`#D1D4DC`(텍스트) 다크 배경 3색을 신규 추가하고, `UP/DOWN/MA/TRENDLINE/SUPPORT_RESISTANCE/RANGE_BOX_COLOR` 전부 밝은 톤으로 교체 + 선 굵기 상수(`MA_LINE_WIDTH` 등) 신규 추가. `static_chart.py`(mplfinance)·`interactive_chart.py`(plotly) 둘 다 동일 배경 다크 스타일 적용, `.pine` 템플릿 3종의 하드코딩 hex도 동일하게 갱신(단일 출처 값 일치 유지) | `visualizer/colors.py` |
...
| `visualizer/` 의존성 격리 | `mplfinance`(pandas/matplotlib/numpy 포함)와 `plotly`를 `[project.dependencies]`가 아니라 `[project.optional-dependencies] visualizer`로만 선언 — 시각화를 안 쓰는 소비 프로젝트가 `uv add git+...`만 하면 이 무거운 의존성을 전혀 받지 않음. 시각화가 필요한 프로젝트는 `uv add "chart-interpreter[visualizer] @ git+..."` 형태로 extra를 명시해야 함(아직 3개 프로젝트 중 실제로 extra를 추가한 곳은 없음) | `pyproject.toml` |
| `interactive_chart.py`의 overlay 계약 | `render_interactive_chart()`는 `static_chart.VALID_OVERLAYS`/`DEFAULT_OVERLAYS`를 새로 정의하지 않고 그대로 import — "어떤 요소를 켤 수 있는지"는 두 렌더러가 공유하는 계약이지 static_chart 전용이 아니므로 단일 출처 원칙(CLAUDE.md)에 따라 중복 정의 금지. `plotly`도 stub/py.typed가 없어 `mplfinance`와 동일하게 `[[tool.mypy.overrides]] module=["mplfinance.*","plotly.*"]`로 처리 | `visualizer/interactive_chart.py`, `pyproject.toml` |
| `interactive_chart.build_figure()` | 2026-07-08, 코단 대시보드 연동 계기로 추가 — `render_interactive_chart()`(HTML 파일 저장)와 `build_figure()`(`go.Figure` 객체만 반환, 파일 I/O 없음)를 분리. 소비 프로젝트가 Streamlit `st.plotly_chart()` 등 자체 UI에 직접 임베드할 때는 `build_figure()`를 쓴다 — `render_interactive_chart()`는 이 함수를 감싸 HTML로 저장하는 얇은 래퍼로 재작성됨(기존 시그니처·동작 무변경). 실제 소비 사례: 코단 `dashboard/streamlit_app.py`의 "추천 리스트 행 클릭 → 분석 패널" 기능(`d:\www\코단-코인단타\docs\HISTORY.md` 2026-07-08 참고) | `visualizer/interactive_chart.py` |
