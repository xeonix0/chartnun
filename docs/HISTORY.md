...
## 2026-07-08 (차트 색상 다크 모드 전면 개편 — 트레이딩뷰 벤치마킹)

- 코단 대시보드에서 실제로 분석 패널을 켜본 사용자가 "차트가 눈에 잘 안 들어옴, 트레이딩뷰 다크모드를 벤치마킹해, 선도 잘 안 보이니 다크모드 색상에 맞게 하고 굵기도 수정해"라고 피드백 — 기존 `visualizer/colors.py` 색상(Material Design 700대 다크 톤: `#2E7D32`/`#C62828`/`#1565C0`/`#EF6C00`/`#8E24AA`/`#7E57C2`)이 흰 배경을 전제로 고른 값이라, 어두운 배경(코단 대시보드의 퍼플 테마, 또는 plotly가 자동 적용하는 다크 배경)에서 채도는 높지만 명도가 낮아 대비가 부족했던 게 원인
- `visualizer/colors.py` 전면 개편: 색상 **계열**(초록=상승/빨강=하락/파랑=이평선/주황=추세선/자주=저항지지/보라=박스권, 스펙 §10-1-1)은 그대로 유지하고 명도만 트레이딩뷰 실제 다크 테마 기준값(양봉 `#26A69A` teal, 음봉 `#EF5350` coral 등, 널리 알려진 공개 정보)을 벤치마킹해 밝은 톤으로 교체: `UP_COLOR=#26A69A`, `DOWN_COLOR=#EF5350`, `MA_COLOR=#42A5F5`, `TRENDLINE_COLOR=#FFA726`, `SUPPORT_RESISTANCE_COLOR=#EA80FC`(기존엔 박스권 보라와 색 계열이 너무 가까웠던 것도 이번에 자주/마젠타 쪽으로 더 벌림), `RANGE_BOX_COLOR=#B388FF`. 다크 배경 3색(`CHART_BACKGROUND_COLOR=#131722`, `CHART_GRID_COLOR=#2A2E39`, `CHART_TEXT_COLOR=#D1D4DC`, 트레이딩뷰 실제 기본 다크 배경/그리드/텍스트 색과 동일)과 선 굵기 4개 상수(`MA_LINE_WIDTH=2.0`/`TRENDLINE_WIDTH=2.5`/`SUPPORT_RESISTANCE_WIDTH=1.5`/`RANGE_BOX_WIDTH=1.5`, 요청받은 "굵기도 수정" 반영) 신규 추가 — 두 렌더러가 배경색까지 공유하도록 단일 출처 범위를 색상에서 "스타일 전반"으로 넓힘
- `interactive_chart.py`(plotly): `build_figure()`의 각 트레이스가 새 굵기 상수 참조하도록 교체, 캔들에 `increasing_fillcolor`/`decreasing_fillcolor` 명시 추가(트레이딩뷰처럼 꽉 찬 캔들), `fig.update_layout()`에 `template="plotly_dark"` + `paper_bgcolor`/`plot_bgcolor`/`font.color`를 다크 3색으로 명시 지정 — Streamlit에 임베드될 때(`st.plotly_chart` 자동 테마 적용)뿐 아니라 `render_interactive_chart()`로 저장한 **독립 HTML을 그냥 브라우저로 열었을 때도** 다크 배경이 되도록(호스트 앱의 암묵적 테마에 의존하지 않음)
- `static_chart.py`(mplfinance): `mpf.make_addplot()` 각 오버레이에 새 굵기 상수 적용. `mpf.make_mpf_style()`에 `facecolor`/`figcolor`/`edgecolor`/`gridcolor`(다크 3색 재사용)와 `rc`에 `text.color`/`axes.labelcolor`/`xtick.color`/`ytick.color`(전부 `CHART_TEXT_COLOR`) 추가 — 기존 `_korean_font_rc()`(폰트 전용, 단일 책임 유지)와 병합해서 넘김. 범례(`axlist[0].legend()`)에 `labelcolor=CHART_TEXT_COLOR` 추가(다크 배경에 검정 범례 텍스트가 묻히는 문제 방지). `fig.savefig()`에 `facecolor=fig.get_facecolor()` 명시 — matplotlib 버전에 따라 저장 시 배경이 흰색으로 되돌아갈 수 있는 문제를 원천 차단
- `pinescript_export/templates/*.pine` 3종의 하드코딩 hex도 동일 값으로 교체(Pine은 `colors.py`를 import할 수 없어 값만 동일하게 유지하는 기존 관례, `docs/HISTORY.md` 2026-07-08 "pinescript_export 구현" 항목 참고) — `pullback.pine`(EMA `#42A5F5`·linewidth 1→2, 눌림 표시 `#26A69A`/`#EF5350`), `range_box.pine`(`#B388FF`, 박스선에 linewidth=2 신규 추가), `trendline.pine`(`#FFA726`, width 2→3)
- 색상 상수를 하드코딩 assert하는 테스트가 없음을 grep으로 확인(`tests/`에 hex·색상 상수명 참조 0건) — 회귀 위험 없이 값만 교체
- **STOP GATE 3 육안 대조**: `scripts/demo_render_chart.py`를 재실행해 새 PNG/HTML을 실제로 생성 후 Read 도구로 PNG를 직접 열어 확인 — 다크 네이비 배경 위에 teal/coral 캔들, 밝은 파랑 EMA, 밝은 주황 추세선, 밝은 마젠타 저항/지지선, 밝은 라벤더 박스권 선이 전부 뚜렷하게 구분됨을 확인(기존 대비 명확히 개선). HTML도 Playwright로 실제 브라우저 스크린샷을 떠서 동일하게 확인 — `template="plotly_dark"` 지정 덕에 Streamlit 없이 파일을 그냥 열어도 다크 배경으로 렌더링됨을 검증
- 게이트 확인: `ruff check .` / `mypy . --strict` / `pytest` 전부 통과 (78 tests, 회귀 없음)
- `docs/MASTER.md` "확정된 설계 계약" 표의 "시각화 색상" 행 갱신(정확한 hex와 변경 이유 기록)
